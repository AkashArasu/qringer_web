...
class _VisitorSession {
  ...
  final String callId,
      ...
      sessionToken,
      streamApiKey;
  factory _VisitorSession.fromJson(Map<String, dynamic> json) =>
      _VisitorSession(
...
class AutomaticDoorbellPage extends StatefulWidget {
  ...
  @override
  State<AutomaticDoorbellPage> createState() => _AutomaticDoorbellPageState();
}
...
class _AutomaticDoorbellPageState extends State<AutomaticDoorbellPage> {
  ...
  void initState() {
    ...
    _pageHideSubscription =
        html.window.onPageHide.listen((_) => _sendUnloadSignal());
    unawaited(_startAutomatically());
  }
  ...
  Future<void> _startAutomatically() async {
    ...
    if (propertyId == null) {
      return _fail('This QR code is invalid or has expired.');
    }
    try {
  ...
  Future<http.Response> _createVisitorSession(String propertyId) async {
    Object? lastError;
    for (var attempt = 1; attempt <= 3; attempt++) {
      try {
        final response = await http
            .post(
              Uri.parse('$_apiBaseUrl/v1/visitor-sessions'),
              headers: {'Content-Type': 'application/json'},
              body: jsonEncode(
                  {'propertyId': propertyId, 'requestId': _requestId}),
            )
            .timeout(const Duration(seconds: 10));
        if (response.statusCode < 500 || attempt == 3) return response;
        lastError = Exception(_errorFrom(response));
      } catch (error) {
        lastError = error;
    ...
    throw lastError ?? StateError('Unable to create visitor session');
  }

  String? _propertyIdFromUrl() {
    final segments = Uri.base.pathSegments;
    final pIndex = segments.indexOf('p');
    if (pIndex >= 0 && segments.length > pIndex + 1) {
  ...
  Future<void> _pollCallState() async {
    ...
    if (session == null || _cancelled || _polling) return;
    _polling = true;
    try {
      final response = await http.get(
          Uri.parse('$_apiBaseUrl/v1/calls/${session.callId}/events'),
          headers: {
      ...
      switch (state) {
        case 'accepted':
          ...
          break;
        case 'busy':
          await _finishFromRemote(VisitorCallStatus.busy);
          break;
        case 'no_answer':
          await _finishFromRemote(VisitorCallStatus.noAnswer);
          break;
        case 'declined':
          await _finishFromRemote(VisitorCallStatus.declined);
          break;
        case 'cancelled':
          await _finishFromRemote(VisitorCallStatus.cancelled);
          break;
        case 'ended':
  ...
  Future<void> _connectMedia() async {
    if (_status == VisitorCallStatus.connected ||
        _status == VisitorCallStatus.connecting) {
      return;
    }
    final session = _session;
    ...
    try {
      ...
      for (final track
          in _permissionStream?.getTracks() ?? <html.MediaStreamTrack>[]) {
        track.stop();
      }
      _permissionStream = null;
      _streamVideo = StreamVideo(session.streamApiKey,
          user: User.regular(userId: session.visitorId, name: 'Visitor'),
          userToken: session.streamToken);
      final connection = await _streamVideo!.connect();
  ...
  void _sendUnloadSignal() {
    ...
    final queued = html.window.navigator.sendBeacon(
      '$_apiBaseUrl/v1/calls/${session.callId}/$action',
      body,
    );
  ...
  void _setStatus(VisitorCallStatus value) {
  ...
  bool _isTerminal(VisitorCallStatus status) => {
        VisitorCallStatus.busy,
        VisitorCallStatus.noAnswer,
        VisitorCallStatus.declined,
        VisitorCallStatus.cancelled,
        VisitorCallStatus.ended,
        VisitorCallStatus.error
      }.contains(status);
  ...
  String _friendlyError(Object error) => error.toString().contains('NotAllowed')
  ...
  Widget build(BuildContext context) => Scaffold(
      body: Container(
          ...
          child: SafeArea(
              child: _status == VisitorCallStatus.connected
                  ? _buildCall()
                  : _buildStatus())));
  Widget _buildCall() => StreamCallContainer(
        ...
        onCallDisconnected: (_) =>
            unawaited(_finishFromRemote(VisitorCallStatus.ended)),
  ...
  Widget _buildStatus() {
    final details = switch (_status) {
      VisitorCallStatus.requestingPermission => (
          'Allow camera and microphone',
          'QROnly needs both permissions to let the homeowner see and hear you.',
          Icons.video_call
        ),
      VisitorCallStatus.preparing => (
          'Starting doorbell',
          'Connecting securely…',
          Icons.doorbell
        ),
      VisitorCallStatus.ringing => (
          'Ringing homeowner',
          'Please wait while they answer.',
          Icons.notifications_active
        ),
      VisitorCallStatus.connecting => (
          'Connecting video',
      ...
      VisitorCallStatus.noAnswer => (
          'No answer',
          'The homeowner did not answer.',
          Icons.phone_missed
        ),
      VisitorCallStatus.declined => (
          'Call declined',
          'The homeowner is unavailable.',
          Icons.call_end
        ),
      VisitorCallStatus.cancelled => (
          'Call cancelled',
          'Your doorbell call was cancelled.',
          Icons.cancel
        ),
      VisitorCallStatus.ended => (
          'Session ended',
          'The doorbell session has ended. You can safely close this window.',
    ...
    return Center(
        child: Padding(
            ...
            child:
              ...
              Icon(details.$3, color: Colors.white, size: 72),
              ...
              Text(details.$2,
                  ...
                  textAlign: TextAlign.center),
              if (_status == VisitorCallStatus.ringing ||
                  _status == VisitorCallStatus.preparing) ...[
...
class _VisitorCallSurfaceState extends State<_VisitorCallSurface> {
  ...
  void _logState(String event, CallState state) {
    ...
    final local =
        participants.where((participant) => participant.isLocal).firstOrNull;
    final remotes =
        participants.where((participant) => !participant.isLocal).toList();
    final localVideoTracks =
        local == null || !local.publishedTracks.containsKey(SfuTrackType.video)
            ? 0
            : 1;
